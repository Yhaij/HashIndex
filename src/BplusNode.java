...
public class BplusNode <K extends Comparable<K>, V> implements Serializable{
    ...
    /** 父节点 */
    protected transient BplusNode<K, V> parent;

    /** 叶节点的前节点*/
    protected transient BplusNode<K, V> previous;

    /** 叶节点的后节点*/
    protected transient BplusNode<K, V> next;
    ...
    /** 叶子节点的信息(解决存储相同key不同value的问题) */
    protected List<LeafNode<K, V>> leafNodes;
    ...
    public BplusNode(boolean isLeaf) {
        ...
        if (!isLeaf) {
            ...
            leafNodes = new ArrayList<LeafNode<K, V>>();
        }
    }
    ...
    public BplusNode getNode(K key){
        if(isLeaf){
            return this;
        }
        if(key.compareTo(entries.get(0).getKey()) < 0){
            ...
            while (low <= high) {
                ...
                if (comp == 0) {
                    return children.get(mid+1).getNode(key);
                } else if (comp < 0) {
                    low = mid + 1;
                } else {
                    high = mid - 1;
            ...
            return children.get(low).getNode(key);
        }
    }

    public List<V> getList(K key){
        BplusNode<K, V> node = getNode(key);
        List<Entry<K, V>> list = node.entries;
        List<LeafNode<K, V>> leafList = node.leafNodes;
        List<V> result = new ArrayList<V>();
        int low = 0, high = list.size() - 1, mid;
            int comp ;
            while (low <= high) {
                mid = (low + high) / 2;
                comp = list.get(mid).getKey().compareTo(key);
                if (comp == 0) {
//                    return entries.get(mid).getValue();
//                    System.out.println("value is :" + list.get(mid).getKey());
                    LeafNode<K,V> p = leafList.get(mid);
                    while (p != null){
    ...
    public void insertOrUpdate(K key, V value, BplusTree<K,V> tree){
        ...
        if (isLeaf){
            ...
            if (parent != null) {
                ...
                leafNodes = null; //删除当前节点的叶子节点队列节点引用
        ...
        for (int i = 0; i < entries.size(); i++) {
            if(leftSize !=0){
                ...
                if(!b&&entries.get(i).getKey().compareTo(key) > 0){
                    ...
                    left.leafNodes.add(new LeafNode<K, V>(key, value));
                    b = true;
                    i--;
                }else {
                    left.entries.add(entries.get(i));
                    left.leafNodes.add(leafNodes.get(i));
                }
            }else {
                if(!b&&entries.get(i).getKey().compareTo(key) > 0){
                    right.entries.add(new SimpleEntry<K, V>(key, value));
                    right.leafNodes.add(new LeafNode<K, V>(key, value));
                    b = true;
                    i--;
                }else {
                    right.entries.add(entries.get(i));
                    right.leafNodes.add(leafNodes.get(i));
                }
            }
        }
        if(!b){
    ...
    protected void insertOrUpdate(K key, V value){
        ...
        while (low <= high) {
            ...
            if (comp == 0) {
                ...
                LeafNode<K, V> newLeafNode = new LeafNode<K, V>(key, value, leafNodes.get(mid).next);
                leafNodes.get(mid).next = newLeafNode;
                break;
            } else if (comp < 0) {
                low = mid + 1;
            } else {
        ...
        if(low>high){
            ...
            leafNodes.add(low, new LeafNode<K ,V>(key, value));
        }
    }
